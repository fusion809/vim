...
if exists("b:is_bash")
 ...
 syn keyword bashStatement add-apt-repository apt apt-cache apt-file apt-get apt-key awk bundle bzcat cat chmod chroot clear complete cp dnf dpkg du egrep expr fgrep find gnufind gnugrep grep gunzip gzip less ln ls mkdir mount mv pacman pkg pip pip2 pip3 rc-add rm rmdir rpm sed sleep sort strip systemctl tail urpme urpmf urpmi urpmi_rpm-find-leaves@ urpmq yum yum-builddep zcat zypper
